...
#         logger.info("Processing batch mode")
#
#         # Check for ZIP file first (iOS/mobile mode)
#         if 'batch_zip' in request.files:
#             zip_file = request.files['batch_zip']
#             if zip_file.filename != '':
#                 logger.info(f"Processing ZIP file: {zip_file.filename}")
#                 return process_zip_batch(zip_file, class_names, max_size, conf_threshold, session_id)
#
...
#
#
# def process_zip_batch(zip_file, class_names, max_size, conf_threshold, session_id):
#     """Process batch upload from ZIP file (for iOS/mobile)"""
#     try:
...
#                         }
#                     except Exception as e:
#                         logger.error(f"Error reading label file {filename}: {str(e)}")
#                         label_contents[base_name] = {
#                             'filename': filename,
#                             'content': None
#                         }
...
#                 ))
#
#         logger.info(f"Found {len(matches)} matching image-label pairs in ZIP")
#
#         if not matches:
#             logger.error("No matching image-label pairs found in ZIP")
#             return jsonify({"error": "No matching image-label pairs found in ZIP file"}), 400
#
#         # Prepare zip file in memory
#         zip_buffer = BytesIO()
#         with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
...
#         # Prepare zip file for download
#         zip_buffer.seek(0)
#         logger.info(f"Completed ZIP batch processing session: {session_id}")
#
#         return send_file(
#             zip_buffer,
#             as_attachment=True,
...
def process():
    ...
    # Batch processing mode
        ...
        # Check for ZIP file first (iOS/mobile mode or fallback)
        if 'batch_zip' in request.files:
            zip_file = request.files['batch_zip']
            if zip_file.filename != '':
                logger.info(f"Processing ZIP file: {zip_file.filename}")
